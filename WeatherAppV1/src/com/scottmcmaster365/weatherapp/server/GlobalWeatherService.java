...
public class GlobalWeatherService {
	...
	public List<String> getCitiesForCountry(String countryName)
		...
		String url = BASE_URL + "GetCitiesByCountry?CountryName="
		...
		NodeList cityNodes = (NodeList) xPath.evaluate("//City", dDoc,
	...
	public Weather getWeatherForCity(String countryName, String cityName)
			throws Exception {
		Weather weather = new Weather();

		String url = BASE_URL + "GetWeather?CityName="
				+ URLEncoder.encode(cityName, "utf-8") + "&CountryName="
				+ URLEncoder.encode(countryName, "utf-8");
		String result = null;
		InputStream in = new URL(url).openStream();
		try {
		...
		// This response is weird -- it has encoded XML embedded inside an XML
		// wrapper.
		...
		XPath xPath = XPathFactory.newInstance().newXPath();
		NodeList locationNodes = (NodeList) xPath.evaluate("//Location", dDoc,
				XPathConstants.NODESET);
		weather.setCityName(locationNodes.item(0).getTextContent());

		NodeList tempNodes = (NodeList) xPath.evaluate("//Temperature", dDoc,
				XPathConstants.NODESET);
		weather.setTemperature(tempNodes.item(0).getTextContent());

		NodeList visNodes = (NodeList) xPath.evaluate("//Visibility", dDoc,
				XPathConstants.NODESET);
		weather.setVisibility(visNodes.item(0).getTextContent());

		NodeList windNodes = (NodeList) xPath.evaluate("//Wind", dDoc,
				XPathConstants.NODESET);
		weather.setWind(windNodes.item(0).getTextContent());

		NodeList pressureNodes = (NodeList) xPath.evaluate("//Pressure", dDoc,
				XPathConstants.NODESET);
		weather.setPressure(pressureNodes.item(0).getTextContent());

		NodeList humidNodes = (NodeList) xPath.evaluate("//RelativeHumidity",
